...
For simplicity, this example of `ViewModelBuilder` doesn’t implement code to handle when the user creates, updates, and or deletes data. The most common way this is dealt with is to use observers (e.g. KVO) or delegates. I typically prefer delegates but there are pros and cons to each approach, this is a topic for another time. Going forward I am excited about trying an [Rx approach](https://medium.com/flawless-app-stories/how-to-use-a-model-view-viewmodel-architecture-for-ios-46963c67be1b) to observers using a project like [Bond](https://github.com/DeclarativeHub/Bond).
